...
def _set_phase_deadline(game_id: str, phase: str, seconds: int):
    deadline = datetime.utcnow() + timedelta(seconds=seconds)
    per_game = _phase_deadlines.setdefault(game_id, {})
    per_game[phase] = deadline
    token = str(uuid.uuid4())
    _phase_tokens[(game_id, phase)] = token
    return deadline, token


def _clear_phase_deadline(game_id: str, phase: str | None = None):
    if phase is None:
        _phase_deadlines.pop(game_id, None)
        for key in list(_phase_tokens.keys()):
            if key[0] == game_id:
                _phase_tokens.pop(key, None)
        return

    per_game = _phase_deadlines.get(game_id)
    if per_game:
        per_game.pop(phase, None)
        if not per_game:
            _phase_deadlines.pop(game_id, None)
    _phase_tokens.pop((game_id, phase), None)
...
def _begin_trading(game, bid_price: float, ask_price: float, auto_generated: bool):
    ...
    game.status = "trading"
    _clear_phase_deadline(game.id, "quoting")
    trading_deadline, trading_token = _set_phase_deadline(game.id, "trading", TRADING_SECONDS)
    db.session.commit()
    ...
    socketio.emit("quotes_set", {
        ...
        "tradingDeadline": _deadline_ms(trading_deadline),
    }, room=game.id)
    _start_prefetch_if_needed(game.id)

    app = current_app._get_current_object()
    socketio.start_background_task(_run_trading_timeout, app, game.id, trading_token)
...
def _settle_game(game_id: str):
    ...
    _clear_phase_deadline(game_id, "trading")
    db.session.commit()
...
def _finish_auction_and_start_quoting(game_id: str):
    ...
    _clear_phase_deadline(game_id, "auction")
    quoting_deadline, quoting_token = _set_phase_deadline(game_id, "quoting", QUOTING_SECONDS)
    db.session.commit()

    socketio.emit("market_maker_selected", {
        "playerName": winner.name,
        "spread": winner_bid.spread,
        "quotingDeadline": _deadline_ms(quoting_deadline),
    }, room=game_id)

    app = current_app._get_current_object()
    socketio.start_background_task(_run_quoting_timeout, app, game_id, quoting_token)
...
def _run_auction_timeout(app, game_id: str, token: str):
    socketio.sleep(AUCTION_BID_SECONDS)
    with app.app_context():
        if _phase_tokens.get((game_id, "auction")) != token:
            return
        _insert_timeout_bids_for_missing_players(game_id)
        result = _finish_auction_and_start_quoting(game_id)
        if result and "error" in result:
            _clear_phase_deadline(game_id, "auction")


def _run_quoting_timeout(app, game_id: str, token: str):
    socketio.sleep(QUOTING_SECONDS)
    with app.app_context():
        if _phase_tokens.get((game_id, "quoting")) != token:
            return
...
def _run_trading_timeout(app, game_id: str, token: str):
    socketio.sleep(TRADING_SECONDS)
    with app.app_context():
        if _phase_tokens.get((game_id, "trading")) != token:
            return
...
def _game_state(game, reveal_price=False):
    ...
    return {
        ...
        "auctionDeadline": _deadline_ms(_phase_deadlines.get(game.id, {}).get("auction")),
        "quotingDeadline": _deadline_ms(_phase_deadlines.get(game.id, {}).get("quoting")),
        "tradingDeadline": _deadline_ms(_phase_deadlines.get(game.id, {}).get("trading")),
        "players": players,
        "trades": trades,
    }
...
def start_auction(game_id):
    ...
    game.status = "auction"
    _clear_phase_deadline(game_id)
    auction_deadline, auction_token = _set_phase_deadline(game_id, "auction", AUCTION_BID_SECONDS)
    db.session.commit()

    app = current_app._get_current_object()
    socketio.start_background_task(_run_auction_timeout, app, game_id, auction_token)

    socketio.emit("auction_started", {
        "gameId": game_id,
        "auctionDeadline": _deadline_ms(auction_deadline),
    }, room=game_id)
    return jsonify({"status": game.status, "auctionDeadline": _deadline_ms(auction_deadline)})
